...
UKF::UKF() {
  ...
  // initialize covariance matrix with known values
...
void UKF::InitializeKF(MeasurementPackage &meas_package) {
    ...
    if (meas_package.sensor_type_ == MeasurementPackage::LASER) {
        ...
        double ro = raw[0], theta = raw[1], ro_dot = raw[2];
...
void UKF::Prediction(double delta_t) {
  MatrixXd Xsig_aug = GenerateAugmentedSigmaPoints(); // generate sigma points (augmented with noise values)
  PredictSigmaPoints(Xsig_aug, delta_t); // predict sigma points: just apply model function
  PredictMeanAndCovariance(); // predict mean & covariance, store into x_ and P_
}
...
MatrixXd UKF::computeCrossCorrelationMatrix(int n_z, MatrixXd &Zsig, VectorXd &z_pred) {
...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
    ...
    for (int i = 0; i < n_sigma_; i++) {
     ...
     MatrixXd Tc = computeCrossCorrelationMatrix(n_laser_, Zsig, z_pred);
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
    ...
    MatrixXd Tc = computeCrossCorrelationMatrix(n_radar_, Zsig, z_pred);
